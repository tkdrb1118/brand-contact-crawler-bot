...
Apps Script 버튼 실행은 `브랜드URL`, `연락처`, `이메일` 중 비어 있는 값이 있는 행을 찾아 최대 30개씩 수집합니다. 이미 세 값이 모두 있는 행은 건너뜁니다.
...
5. 이후 `브랜드DB수집` 시트의 `수집 실행` 체크박스를 누를 때마다 최대 30개 업체를 수집합니다.
6. 시트가 bound script로 연결된 경우에는 새로고침 후 상단 메뉴 `브랜드 DB 수집`도 사용할 수 있습니다.
7. 실행 후 `브랜드DB수집!B5`에서 `행 범위 / 처리 수 / 업데이트 수 / 제외 수 / 무효URL 수`를 확인합니다.
...
- 이미 브랜드URL/연락처/이메일이 모두 있는 행은 건너뛰고, 셋 중 하나라도 비어 있는 행부터 버튼 실행 시 최대 30개씩 수집합니다.
