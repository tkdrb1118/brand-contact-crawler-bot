...
- Apps Script 단독 실행: 시트 안의 실행 체크박스를 누를 때마다 기존 빈값 보강 또는 신규 브랜드 DB 발굴을 최대 30개 수행합니다.
- Node.js/Codex 실행: 로컬이나 Codex에서 `npm run crawl`, `npm run worker`로 실행합니다.

Apps Script 버튼 실행은 먼저 `브랜드URL`, `연락처`, `이메일` 중 비어 있는 기존 행을 보강하고, 보강 대상이 없으면 네이버 브랜드스토어/스마트스토어 후보를 발굴해 시트 마지막 브랜드 행 아래에 신규 브랜드 DB를 추가합니다. 신규 발굴 시에는 브랜드명/브랜드URL을 먼저 추가하고, 이후 실행에서 연락처/이메일을 보강합니다.
컨트롤 시트의 `처리`는 이번 실행에서 확인한 업체 수이고, `업데이트`는 그중 실제로 셀 값이 바뀐 업체 수입니다.
...
5. 이후 `브랜드DB수집` 시트의 `수집 실행` 체크박스를 누를 때마다 최대 30개 업체를 보강하거나 신규 발굴합니다.
...
- `30개 즉시 수집`: 현재 지정된 시트에서 기존 빈값 보강 또는 신규 브랜드 DB 발굴을 바로 실행합니다.
...
- 이미 브랜드URL/연락처/이메일이 모두 있는 행은 건너뜁니다. 보강 대상이 없으면 네이버 브랜드스토어/스마트스토어 후보를 발굴해 마지막 브랜드 행 아래에 신규 행을 추가합니다.
